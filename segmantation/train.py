...
def train(cfg): 
   ...
   #     save_top_k=3,  # Сохраняем только 3 лучших модели
    ...
    #)
    trainer = instantiate(cfg['trainer'], callbacks=[instantiate(cfg.callbacks[clb_name]) for clb_name in cfg.callbacks]) #[instantiate(cfg.callbacks[clb_name]) for clb_name in cfg.callbacks]) 
